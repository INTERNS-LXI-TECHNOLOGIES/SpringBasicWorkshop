...
public class DatabaseRepo {
	...
	public void addPatient(Patient p) {
		...
		try {
			s = (Statement) con.createStatement();
			s.executeUpdate("insert into Patient values('" + p.getName() + "','" + p.getPhoneNumber() + "','"
					+ p.getAddress() + "','" + p.getDoctorName() + "')");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public ArrayList<Patient> getPatientRecords() 
	{
		...
		try {
			...
			while (r.next()) 
			{
				patients.add(i, new Patient());
				patients.get(i).setName(r.getString("name"));
				patients.get(i).setPhoneNumber(r.getString("Number"));
				patients.get(i).setAddress(r.getNString("address"));
				patients.get(i).setDoctorName(r.getString("selecteddoctor"));
				i++;
			}
		} 
		catch (SQLException e) 
